...
#define TD_INTERNAL_COMPONENT_LOAD_DATA(...) struct internal_load_data { \
		TD_APPLY_EACH_CONCAT(TD_INTERNAL_COMPONENT_FIELD, __VA_ARGS__) \
	};
...
#define TD_INTERNAL_COMPONENT_LOAD(...) void Load(const void* data) override { \
		...
		TD_APPLY_EACH_CONCAT(TD_INTERNAL_COMPONENT_LOAD_FIELD, __VA_ARGS__) \
	}
...
#define TD_INTERNAL_COMPONENT_CREATE_LOAD_DATA(...) \
	static bool CreateLoadData(internal_load_data* loadData, const Deserializer& deserializer) { \
		TD_APPLY_EACH_CONCAT(TD_INTERNAL_COMPONENT_CREATE_LOAD_DATA_FIELD, __VA_ARGS__) \
		return true; \
	}
...
#define TD_INTERNAL_COMPONENT_DEBUG(...) void Debug() override { \
		TD_APPLY_EACH_CONCAT(TD_INTERNAL_COMPONENT_DEBUG_FIELD, __VA_ARGS__) \
	}

#define TD_COMPONENT(...) TD_APPLY_EACH_CONCAT(TD_INTERNAL_COMPONENT_FIELD, __VA_ARGS__) \

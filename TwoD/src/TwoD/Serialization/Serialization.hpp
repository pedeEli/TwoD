...
#define TDI_STRUCT_DEF(meta, fields, ...) \
	...
	struct TDI_GET_NAME(meta) TDI_GET_BASE_DEF(meta) { \
	public: \
		TD_APPLY_EACH_CONCAT(TDI_GET_FIELD_DEF, TD_UNWRAP fields) \
		__VA_ARGS__ \
	};
...
// enum defs
#define TDI_ENUM_FIELD_DEF(field) TDI_GET_NAME(field) TD_IF_ELSE((TDI_GET_INIT(field)), = TD_UNWRAP TDI_GET_INIT(field), )
#define TDI_ENUM_DEF(meta, fields) \
	enum class TDI_GET_NAME(meta) TDI_GET_BASE_DEF(meta) { \

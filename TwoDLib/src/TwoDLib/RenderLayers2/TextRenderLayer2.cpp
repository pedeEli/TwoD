...
namespace TwoD
{
	void TextRenderLayer2::Init()
	{
		m_shader = &App::Get<AssetManager>().Get<Shader>("TwoDLib::TextRenderer2");
	}

	void TextRenderLayer2::Bind(SDL::CommandBuffer* commandBuffer, SDL::RenderPass* renderPass) const
	{
		m_shader->Bind(renderPass);
	}

	void TextRenderLayer2::Render(const ECS& ecs, RenderSystem2& renderSystem, SDL::RenderPass* renderPass, size_t index)
	{
	...
	void TextRenderLayer2::Update(const ECS& ecs)
	{
	...
	const std::vector<std::type_index>& TextRenderLayer2::GetRendererTypes() const
	{
		return s_types;
	}
}
#include "tdpch.hpp"
#include "SpriteRenderLayer2.hpp"
...
namespace TwoD
{
	void SpriteRenderLayer2::Init()
	{
		m_shader = &App::Get<AssetManager>().Get<Shader>("TwoDLib::SpriteRenderer2");
		m_spriteAtlas = &App::Get<AssetManager>().Get<SpriteAtlas>("sprite-atlas");
		//m_spriteAtlas->Pack();
	}

	void SpriteRenderLayer2::Bind(SDL::CommandBuffer* commandBuffer, SDL::RenderPass* renderPass) const
	{
		m_shader->Bind(renderPass);
	}

	void SpriteRenderLayer2::Render(const ECS& ecs, RenderSystem2& renderSystem, SDL::RenderPass* renderPass, size_t index)
	{
	...
	void SpriteRenderLayer2::Update(const ECS& ecs)
	{
	...
	const std::vector<std::type_index>& SpriteRenderLayer2::GetRendererTypes() const
	{
		return s_types;
	}
}